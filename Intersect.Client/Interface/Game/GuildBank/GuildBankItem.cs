...
namespace Intersect.Client.Interface.Game.GuildBank
{
    ...
    public class GuildBankItem
    {
        ...
        public GuildBankItem(GuildBankWindow guildbankWindow, int index)
        {
            mGuildBankWindow = guildbankWindow;
            mMySlot = index;
        }

        public void Setup()
        {
            Pnl = new ImagePanel(Container, "GuildBankItemIcon");
        ...
        {
            if (Globals.InGuildBank)
            {
                Globals.Me.TryWithdrawGuildItem(mMySlot);
            }
        }
        ...
        void pnl_HoverEnter(Base sender, EventArgs arguments)
        {
            ...
            if (Globals.GuildBank[mMySlot]?.Base != null)
            {
                mDescWindow = new ItemDescWindow(
                    Globals.GuildBank[mMySlot].Base, Globals.GuildBank[mMySlot].Quantity, mGuildBankWindow.X, mGuildBankWindow.Y,
                    Globals.GuildBank[mMySlot].StatBuffs
                );
            }
        }
        ...
        public void Update()
        {
            if (Globals.GuildBank[mMySlot].ItemId != mCurrentItemId)
            {
                mCurrentItemId = Globals.GuildBank[mMySlot].ItemId;
                var item = ItemBase.Get(Globals.GuildBank[mMySlot].ItemId);
                if (item != null)
                {
            ...
            else
            {
                if (mDragIcon.Update())
                {
                    ...
                    if (mGuildBankWindow.RenderBounds().IntersectsWith(dragRect))
                    {
                        for (var i = 0; i < Options.MaxBankSlots; i++)
                        {
                            if (mGuildBankWindow.Items[i].RenderBounds().IntersectsWith(dragRect))
                            {
                                if (FloatRect.Intersect(mGuildBankWindow.Items[i].RenderBounds(), dragRect).Width *
                                    FloatRect.Intersect(mGuildBankWindow.Items[i].RenderBounds(), dragRect).Height >
                                    bestIntersect)
                                {
                                    bestIntersect =
                                        FloatRect.Intersect(mGuildBankWindow.Items[i].RenderBounds(), dragRect).Width *
                                        FloatRect.Intersect(mGuildBankWindow.Items[i].RenderBounds(), dragRect).Height;
                        ...
                        if (bestIntersectIndex > -1)
                        {
                            ...
                            {
                                //Try to swap....
                                PacketSender.SendMoveGuildBankItems(bestIntersectIndex, mMySlot);
